...
/// Options for `numan setup loader` tool management and installation.
...
/// A single tool entry persisted in `loader-config.nu`.
...
/// Read and parse the tool entries from `loader-config.nu`.
...
/// Parse a nuon-style list of `{ name, command }` records from loader config text.
...
/// Render tool entries into the nuon list format consumed by `loader-config.nu`.
pub fn render_loader_config(entries: &[LoaderConfigEntry]) -> String {
    ...
    for e in entries {
        if validate_tool_name(&e.name).is_err() {
            eprintln!(
                "Warning: dropping invalid tool name '{}' from loader config.",
                e.name
            );
            continue;
        }
...
/// Atomically write tool entries to `loader-config.nu`.
...
/// True when `config.nu` content already sources the vendored loader.nu.
