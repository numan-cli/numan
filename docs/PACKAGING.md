...
2. After the tag-triggered Release workflow succeeds, the [`Publish to WinGet`](../.github/workflows/winget.yml) workflow generates and submits the update PR using the published Windows `.zip` asset. It requires the repository's `WINGET_TOKEN` secret and the existing `tonythethompson/winget-pkgs` fork. Manual dispatch remains available as a recovery path.
