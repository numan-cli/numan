...
2. After the tag-triggered Release workflow completes, the [`Publish to WinGet`](../.github/workflows/winget.yml) workflow verifies the non-draft GitHub Release and its published Windows `.zip` asset, then generates and submits the update PR. It requires the repository's `WINGET_TOKEN` secret and the existing `tonythethompson/winget-pkgs` fork. Manual dispatch remains available as a recovery path.
